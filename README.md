...
Dialog is a modular `VoIP` ➞ `STT` ➞ `AI Agent` ➞ `TTS` ➞ `VoIP` implementation that uses conversational AI for handling voice calls.  It works quite well as it is - and it is also a work in progress.  Dialog provides example implementations for each of the artifacts that comprise a VoIP Agent application. You can use an implementation _as-is_, subclass it, or implement your own.
